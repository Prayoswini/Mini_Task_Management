...
2. **Seed Initial Database Data (Admin & Demo Users)**:
   ...
   *Creates default accounts:*
   - **Admin Account**: Email: `admin@gmail.com` | Password: `password123`
   - **User Accounts**: Email: `john@gmail.com` (or `sophia@gmail.com`, `jane@gmail.com`, `robert@gmail.com`, `emily@gmail.com`) | Password: `password123`
...
1. **User Authentication Model**: User registration is disabled for end-users to enforce organizational security; accounts are provisioned via administrative seeding (`npm run seed`) or admin user management.
2. **Database Storage**: MongoDB Atlas cloud cluster (or local MongoDB instance) is used for data persistence via Mongoose ODM.
3. **CORS & Environment**: The backend API runs on port 5000 (`http://localhost:5000`) and permits cross-origin requests from the React frontend dev server (`http://localhost:5173`).
4. **Token Expiration**: JWT authentication tokens expire in 30 days by default (`JWT_EXPIRE=30d`).
